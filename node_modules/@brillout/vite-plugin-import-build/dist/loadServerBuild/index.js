...
const importBuildPromise_1 = require("./importBuildPromise");
async function loadServerBuild(outDir) {
    ...
    (0, utils_1.assertUsage)(autoImporter.status !== 'DISABLED', "As a library author, make sure your library doesn't call loadServerBuild() when using `inject: true`");
    let success = false;
    let requireError;
    ...
    if (autoImporter.status === 'SET') {
        try {
            ...
            await globalThis[importBuildPromise_1.importBuildPromise];
            success = true;
        }
        catch (err) {
            if (debugLogs_1.DEBUG) {
                requireError = err;
            }
            else {
                throw err;
